 ...
 *  also undirected as whenever an edge is added, the dual edge is also added.
 *
 *  @author Antares Chen
 *  @since  2015-12-25
 */
public class Graph {
    ...
    public void addEdge(Edge e) {
        addEdgeHelper(e.getSource(), e.getDest(), e.getLabel());
    }

    /** Creates an edge between V1 and V2 with no edge weight. */
    public void addEdge(int v1, int v2) {
        addEdgeHelper(v1, v2, 0);
    }

    /** Creates an edge between V1 and V2 with edge weight LABEL. */
    public void addEdge(int v1, int v2, int label) {
        addEdgeHelper(v1, v2, label);
    }
    ...
    /** A helper function that adds a new edge from V1 to V2 with LABEL as the
     *  label. */
    private void addEdgeHelper(int v1, int v2, int label) {
        addVertex(v1);
        addVertex(v2);
