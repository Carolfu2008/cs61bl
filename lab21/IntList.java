...
public class IntList {
    ...
    private IntListNode insert(IntListNode p, IntListNode head) {
        // TODO your code here!
        if (head == null){
            head = new IntListNode(p.item);
        } else {
            for (IntListNode q = head; q != null; q = q.next){
                if (q.item > p.item){
                    if (q == head){
                        head = new IntListNode(p.item, null, head);
                        head.next.prev = head;
                        break;
                    } else {
                        q = new IntListNode(p.item, q.prev, q);
                        q.next.prev = q;
                        q.prev.next = q;
                        break;
                    }
                } else if (q.next == null){
                    q.next = new IntListNode(p.item,q,null);
                    break;
                }
            }
        }
    ...
    public IntList quicksort() {
        ...
        for (IntListNode p = head.next; p != null; p = p.next) {
            if (p.item >= head.item) {
                largeElements.addToEnd(p.item);
            } else {
                smallElements.addToEnd(p.item);
            }
        }
        smallElements.quicksort().addToEnd(head.item);
...
*/
        ...
        // TODO your code here!
        for (IntListNode p = head; p != null; p = p.next) {
            if (oneHalf.size < size/2) {
                oneHalf.addToEnd(p.item);
            } else {
                otherHalf.addToEnd(p.item);
            }
        }
